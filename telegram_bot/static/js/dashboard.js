...
let currentUnifiedFilter = 'active'; // 'active' or 'completed'
let currentPage = 1;
...
async function fetchUnifiedData() {
    ...
    let taskEndpoint;
    if (currentUnifiedFilter === 'active') {
        taskEndpoint = '/api/tasks/running?active_only=true';
    } else if (currentUnifiedFilter === 'completed') {
        taskEndpoint = '/api/tasks/completed';
    } else if (currentUnifiedFilter === 'all') {
        taskEndpoint = '/api/tasks/all';
    }
    taskEndpoint += `&page=${currentPage}&page_size=${pageSize}`;

    // Fetch sessions based on filter
    let sessionEndpoint;
    if (currentUnifiedFilter === 'active') {
        // Active sessions: last activity within 5 minutes
        sessionEndpoint = '/api/metrics/cli-sessions?minutes=5';
    } else if (currentUnifiedFilter === 'completed') {
        // Completed sessions: no activity for 30+ minutes, within last 24 hours
        sessionEndpoint = '/api/metrics/cli-sessions?hours=24';
    } else if (currentUnifiedFilter === 'all') {
        // All sessions within last 7 days
        sessionEndpoint = '/api/metrics/cli-sessions?hours=168';
    }
    sessionEndpoint += `&page=${currentPage}&page_size=${pageSize}`;
...
function renderSessionCard(session) {
    ...
    return `<div class="task-card ${statusClass}" onclick="showSessionDetail('${session.session_id}')">
        <div class="task-header">
...
function renderTaskToolUsage(toolData) {
    ...
    // Auto-scroll to bottom (like a terminal/logfile)
    setTimeout(() => {
...
function renderTerminalToolCall(call, lineNumber) {
    ...
    // Build tool signature - single line format with metadata
    let signature = '';
    let metadata = '';  // Additional info in parentheses

    if (tool === 'Bash') {
        // Use parameters.command directly, fall back to previewObj if not available
        ...
        if (cmd.length > 80) {
            ...
            if (lines.length > 1) {
                // Multi-line command (e.g., heredoc, git commit)
                const firstLine = lines[0].length > 60 ? lines[0].substring(0, 60) + '...' : lines[0];
                displayCmd = `${firstLine} (${lines.length} lines)`;
            } else {
                // Single line - truncate but show beginning
                displayCmd = cmd.substring(0, 77) + '...';
            }
        }

        // Show command in monospace code style without "Bash(command: ...)" wrapper
        signature = `<span class="tool-name-text">Bash</span> <code style="color: #58a6ff; background: #1a1a1a; padding: 0.125rem 0.375rem; border: 1px solid #2a2a2a; font-family: ui-monospace, Monaco, 'Courier New', monospace; font-size: 0.75rem;">${escapeHtml(displayCmd)}</code>`;

        const stdout = previewObj?.stdout || '';
        if (stdout) {
            const lines = stdout.split('\n').length;
            metadata = `${lines} line${lines !== 1 ? 's' : ''} output`;
        }
    } else if (tool === 'Read') {
        // Use parameters.file_path, fall back to previewObj
        const filePath = parameters.file_path || previewObj?.filePath || previewObj?.file?.filePath || 'file';
        const relativePath = toRelativePath(filePath);
        signature = `<span class="tool-name-text">Read</span> <span class="detail-path">${escapeHtml(relativePath)}</span>`;
        if (outputLength > 0) {
            const lines = Math.round(outputLength / 80); // Rough estimate
            metadata = `${lines} lines`;
        }
    } else if (tool === 'Write') {
        // Use parameters.file_path
        const filePath = parameters.file_path || previewObj?.filePath || previewObj?.file?.filePath || 'file';
        const relativePath = toRelativePath(filePath);
        signature = `<span class="tool-name-text">Write</span> <span class="detail-path">${escapeHtml(relativePath)}</span>`;
        if (outputLength > 0) {
            metadata = formatBytes(outputLength);
        }
    } else if (tool === 'Edit') {
        // Use parameters.file_path
        const filePath = parameters.file_path || previewObj?.filePath || previewObj?.file?.filePath || 'file';
        const relativePath = toRelativePath(filePath);
        signature = `<span class="tool-name-text">Edit</span> <span class="detail-path">${escapeHtml(relativePath)}</span>`;
        // Check if we have diff info
        if (previewObj?.additions !== undefined && previewObj?.deletions !== undefined) {
            metadata = `+${previewObj.additions} -${previewObj.deletions}`;
        }
    } else if (tool === 'Grep') {
        // Use parameters.pattern
        const pattern = parameters.pattern || previewObj?.pattern || 'pattern';
        const truncPat = pattern.length > 40 ? pattern.substring(0, 40) + '...' : pattern;
        signature = `<span class="tool-name-text">Grep</span> <code style="color: #58a6ff;">"${escapeHtml(truncPat)}"</code>`;
        if (outputLength > 0) {
            metadata = `${outputLength} matches`;
        } else {
            metadata = 'no matches';
        }
    } else if (tool === 'Glob') {
        // Use parameters.pattern
        const pattern = parameters.pattern || previewObj?.pattern || 'pattern';
        signature = `<span class="tool-name-text">Glob</span> <code style="color: #58a6ff;">"${escapeHtml(pattern)}"</code>`;
        if (outputLength > 0) {
            metadata = `${outputLength} files`;
        }
    } else if (tool === 'Task') {
        ...
        const prompt = parameters.prompt || previewObj?.prompt || '';

        // Smart truncation for prompt like Bash command
        ...
        if (prompt.length > 80) {
            ...
            if (lines.length > 1) {
                // Multi-line prompt
                const firstLine = lines[0].length > 60 ? lines[0].substring(0, 60) + '...' : lines[0];
                displayPrompt = `${firstLine} (${lines.length} lines)`;
            } else {
                // Single line - truncate but show beginning
                displayPrompt = prompt.substring(0, 77) + '...';
            }
        }

        // Build display with description, subagent type, and prompt preview
        let taskDisplay = escapeHtml(description);
        if (subagentType) {
            taskDisplay += ` (${escapeHtml(subagentType)})`;
        }
        if (displayPrompt) {
            taskDisplay += ` - ${escapeHtml(displayPrompt)}`;
        }

        // Show task in monospace code style similar to Bash
        signature = `<span class="tool-name-text">Task</span> <code style="color: #58a6ff; background: #1a1a1a; padding: 0.125rem 0.375rem; border: 1px solid #2a2a2a; font-family: ui-monospace, Monaco, 'Courier New', monospace; font-size: 0.75rem;">${taskDisplay}</code>`;

        // Add metadata about execution
        if (outputLength > 0) {
            metadata = formatBytes(outputLength) + ' output';
        }
        ...
        if (todos.length > 0) {
            ...
            const todoParts = displayTodos.map(todo => {
                const icon = statusIcons[todo.status] || '○';
                const color = statusColors[todo.status] || '#8b949e';
                const content = todo.content.length > 50 ? todo.content.substring(0, 50) + '...' : todo.content;
                return `<span style="color: ${color};">${icon}</span> ${escapeHtml(content)}`;
            });

            let todoDisplay = todoParts.join('<br>');

            // Add "... and N more" if there are more todos
            if (todos.length > 3) {
                todoDisplay += `<br><span style="color: #8b949e;">... and ${todos.length - 3} more</span>`;
            }

            signature = `<span class="tool-name-text">TodoWrite</span> <div style="margin-left: 1rem; margin-top: 0.25rem; font-size: 0.75rem; line-height: 1.5;">${todoDisplay}</div>`;

            // Metadata: count by status
            const completed = todos.filter(t => t.status === 'completed').length;
            const inProgress = todos.filter(t => t.status === 'in_progress').length;
            const pending = todos.filter(t => t.status === 'pending').length;

            metadata = `${completed}/${todos.length} completed`;
        } else {
            signature = `<span class="tool-name-text">TodoWrite</span> <span style="color: #8b949e;">empty</span>`;
        }
    } else {
        signature = `<span class="tool-name-text">${escapeHtml(tool)}</span>`;
        if (outputLength > 0) {
            metadata = formatBytes(outputLength);
        }
    }

    // Build HTML - single line format
    let html = `<div class="tool-call-entry">`;

    // Single line with bullet, timestamp, signature, and metadata
    html += `<div class="tool-call-header">`;
    // Add color class to bullet based on error status
    const bulletClass = hasError ? 'tool-bullet error' : 'tool-bullet';
    html += `<span class="${bulletClass}">●</span>`;

    // Add timestamp if available
    if (timestamp) {
        const timeStr = formatTimestamp(timestamp);
        html += `<span class="tool-timestamp">${timeStr}</span>`;
    }

    html += `<span class="tool-call-signature">${signature}</span>`;

    // Add metadata in parentheses if available
    if (metadata) {
        html += ` <span class="tool-metadata" style="color: #8b949e;">(${metadata})</span>`;
    }

    // Add error indicator inline if present
    if (hasError) {
        const errorMsg = previewObj?.error || 'Unknown error';
        const prefix = errorMsg === 'Unknown error' ? '' : 'ERROR: ';
        html += ` <span style="color: #f85149;">${prefix}${escapeHtml(errorMsg)}</span>`;
    }
...
function renderDocsList() {
    ...
    list.innerHTML = '<div class="tool-list">' + paginatedDocs.map(doc => {
        ...
        return `
            <div class="tool-item doc-item">
                <div class="doc-item-content" onclick="showDocContent('${escapeHtml(doc.path)}')">
                    <div class="tool-name">${escapeHtml(doc.name)}</div>
